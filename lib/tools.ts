...
export const getProductDetails = tool({
  description: `Get detailed information about a specific product in the Vercel swag store. Use this when the user asks for more information about a product, such as its description, price, stock availability, or images.`,
  inputSchema: z.object({
    id: z
      .string()
      .describe('The ID or slug of the product to retrieve details for.'),
  }),
  execute: async ({ id }) => {
    'use step';
    ...
    try {
      ...
      return {
        ...
        stockInfo: await getProductStock(product.id)
      };
    } catch (err) {
      const message =
        err instanceof ApiRequestError ? err.message : 'Unknown error';
